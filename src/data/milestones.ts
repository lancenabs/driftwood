...
// `art` (any variant): a story still at public/story/act{N}/… that plays
// above the beat with the slow Ken Burns zoom (ASSET_PROMPTS_V3). A missing
// file renders nothing — text carries the scene (the auto-light law).
export type Beat =
  | { kind: 'narration'; text: string; when?: BeatCondition; art?: string }
  | { kind: 'robot'; who: 'skip' | 'hollow' | 'echo2' | 'bailer' | 'collier'; text: string; when?: BeatCondition; art?: string }
  | { kind: 'choice'; id: string; prompt: string; when?: BeatCondition; art?: string;
...
export const MILESTONES: Milestone[] = [
  ...
  M({
    ...
    opening: [
      { art: '/story/act1/07_skip.jpg', kind: 'narration', text: 'Day one. The family walks the beach shouting for the ship\'s boats until their voices go ragged. No crew. No wreckage worth a signal. No funnels on any horizon. The strange compass still points inland, and nobody wants to say what nobody is saying: it is 1954, the sea lane is empty, and there is no help coming. Whatever happens next, they are it.' },
  ...
  M({
    ...
    opening: [
      ...
      { art: '/story/act1/09_charcoal_contract.jpg', kind: 'narration', when: { flag: 's1_the_pledge', is: 'together' }, text: 'Together. The word lands in the middle of the family like the first plank of something. Skip writes it — laboriously, tongue-out, in charcoal on a plank: CHALLENGE LOG — THEM. He underlines THEM twice.' },
      { art: '/story/act1/09_charcoal_contract.jpg', kind: 'narration', when: { flag: 's1_the_pledge', is: 'skeptic' }, text: '"This is ridiculous," someone mutters, and someone else laughs — the first laugh since the boat — and Skip solemnly writes RIDICULOUS. BUT IN. at the top of a charcoal plank, and that is somehow the truest contract this family has signed in years.' },
    ],
    instrument: {
  ...
  M({
    ...
    opening: [
      ...
      { kind: 'narration', text: 'Then someone finds it, wedged under a hull plank: a sea chest off the Halcyon. Latched, dry inside. Fishing line, hooks, a fire-steel, a folded photograph gone soft with salt — a family, but not YOUR family. Four faces, sun-squinting, on a deck. On the back, one word in pencil: "us."' },
      { kind: 'choice', id: 's1_tackle_box', prompt: 'A stranger\'s sea chest, and someone else\'s family in a photograph. What does yours do with it?',
        options: [
          { id: 'use', label: 'Use the tools — whoever packed it would want them keeping you alive' },
          { id: 'cairn', label: 'Build a cairn for the chest — tools borrowed, photo kept safe' },
        ] },
      { kind: 'narration', when: { flag: 's1_tackle_box', is: 'use' }, text: 'The vote is quiet and unanimous: somebody lashed this chest tight so it would live, the way somebody lashed your hands to a line. The tools go to work — the line to the fishing rocks, the fire-steel to the pit keeper. The photograph gets a dry pocket and a promise: whoever "us" is, the chest goes back to them. Somehow.' },
  ...
  M({
    ...
    opening: [
      { art: '/story/act1/10_night_watch.jpg', kind: 'narration', text: 'Night one of real watches. Every worry the day was too busy for arrives at once, the way worries do — in a crowd, after dark. The watch-keeper\'s trick, taught by a robot who bails dry boats: write each worry on a leaf, set it in the WATCH PILE, and stand the watch on what is actually in front of you.' },
  ...
  M({
    ...
    opening: [
      { art: '/story/act2/11_village.jpg', kind: 'narration', text: 'Season two opens with an invitation nobody expected: Skip, scrubbed and standing at attention, flanked by a robot built like a furnace with hands like anvils. The Collier. He looks at the family the way a master builder looks at warped lumber — skeptically, but already planning.' },
  ...
  M({
    ...
    opening: [
      { art: '/story/act2/14_sleeping_spiral.jpg', kind: 'narration', text: 'Where does each person sleep? Together, apart, near, far — the camp layout is the family\'s closeness map drawn in sand. There is no right answer; there is only the HONEST one, and it\'s allowed to change.' },
  ...
  M({
    ...
    opening: [
      { art: '/story/act2/16_boys_knife.jpg', kind: 'narration', text: 'The Collier calls it weather-sense: the village storm-proofs on the CALM days, because lashing a roof in the wind is just expensive praying. The family works the camp — extra lashings, deeper stakes, the fire\'s dry-store — and it\'s while scouting deadfall up the north path that someone finds it.' },
      { kind: 'narration', text: 'A campsite. Old. Cold. A stone ring gone green, a lean-to collapsed into its own bones — and driven into the log beside the fire ring, waiting DECADES for a hand its size: a knife. Bone-handled, sea-rusted — and small. A boy\'s knife, its handle carved and re-carved to fit fingers that never grew.' },
      ...
      { kind: 'choice', id: 's2_knife', prompt: 'A boy\'s knife — DECADES old, in an island campsite. Somebody lived here long before you. What now?',
        options: [
      ...
      { kind: 'narration', when: { flag: 's2_knife', is: 'follow' }, text: 'The trail runs inland, toward the falls — and dies at a rockslide older than the trail. Whoever walked it, the island closed the road years ago. The compass in camp, when you check it that night, still points the same way. Patient as a held breath.' },
      ...
      { kind: 'narration', text: 'That night, the fire burns with the sea chest\'s fire-steel and the storm-lashings hold in the first hard gusts of the season. A stranger\'s tools, a boy\'s knife, a compass that points the wrong way — the island keeps handing this family other people\'s kept things. Wherever the trail goes, it goes THROUGH this family now.' },
    ],
    instrument: {
  ...
  M({
    ...
    opening: [
      { art: '/story/act2/17_remembering_house.jpg', kind: 'narration', text: 'Season two closes in the village\'s longest hut — the one the Jumble calls the Remembering House. The family is invited in for the first time. The walls are carved, floor to eave, in patient knife-work — and every panel is the same person: a BOY. A boy alone in a rowboat. A boy building a wooden figure by lamplight, again and again, ten times over. A boy at a forge, small against the fire. Nobody explains. The Collier stands in the doorway with his hat off, if he had a hat, and the family understands they are standing inside someone\'s memory of being loved.' },
  ...
  M({
    ...
    opening: [
      { art: '/story/act3/18_volley.jpg', kind: 'narration', text: 'Season three opens with the strangest sound the island has made yet: laughing. The camp works now — water, walls, the fire that takes on the first breath. And into the ease walks Skip, carrying a woven ball with the ceremony of a crown.' },
  ...
  M({
    ...
    opening: [
      { art: '/story/act3/20_bottle_ashore.jpg', kind: 'narration', text: 'The morning of story circle, the tide brings a guest: a green glass bottle, wax-sealed, riding the swell into the cove like it had the address. Inside — two letters, salt-stiff, in two different hands.' },
      { kind: 'narration', text: 'The first is from a boy named Chip, to an island, to a workshop, to a father: thank-you words from someone who made it. The second is stranger — a captain, writing to their own future self: "there will be hard weeks; this is what today proves." At the bottom, under both signatures, a line in the boy\'s hand: "If a family finds this — the crew says: keep each other. It\'s the whole trick. — C." Nobody aboard the Halcyon ever knew a Chip. The paper is crisp where it should be brittle, and the date under the wax makes no sense at all. Hollow reads it twice and hands it back without one word of explanation, because the sea does not do chronology, and the Jumble learned long ago not to argue with what the tide decides to deliver. Some letters arrive before they are written. The good ones usually do.' },
  ...
  M({
    ...
    opening: [
      { art: '/story/act3/21_signal_unlit.jpg', kind: 'narration', text: 'The cliff-top signal fire is the season\'s biggest build — a tower of dry wood, visible for miles, kept ready but unlit. The Collier supervises with unusual gravity, and halfway through the work the family realizes why: the Jumble has maintained a signal fire on this cliff for YEARS. Fresh-stocked. Never lit.' },
  ...
  M({
    ...
    opening: [
      { art: '/story/act3/22_feast.jpg', kind: 'narration', text: 'Bailer announces it at dawn with the full foghorn: THE FEAST OF SMALL THINGS. The Jumble\'s oldest festival. Rules: no big dishes, no grand gestures — the table is built entirely from tiny good things, each one presented OUT LOUD with its story. The first shell of the day. The joke that landed. The nap.' },
  ...
  M({
    ...
    opening: [
      { art: '/story/act4/24_naming_undertow.jpg', kind: 'narration', text: 'Rebuilding starts — planks re-lashed, larder dried — and the Undertow, named now, becomes visible in the daylight: when the hard talk starts, who in this family CHASES (louder, closer, fix it NOW), and who HIDES (quieter, farther, later, gone)? The Jumble sorts itself shamelessly for the demonstration.' },
  ...
  M({
    ...
    opening: [
      { art: '/story/act4/25_repair_rope.jpg', kind: 'narration', text: 'The Collier brings out the village\'s strangest tool: a rope of many splices — dozens of them, knot after knot, each one wrapped in a different lashing. It is, he explains, ONE rope. It has never been replaced. It has only ever been repaired.' },
  ...
  M({
    ...
    opening: [
      { art: '/story/act4/26_search_line.jpg', when: { flag: 'composition', is: 'family' }, kind: 'narration', text: 'The storm\'s tail floods the creek at dusk, and in the scramble to move the camp uphill, the count comes up short. One head. The youngest. Gone — and the sand by the north path says WHERE, and the where stops every heart in the camp: small footprints, heading inland. Toward the quiet camp. Toward the compass\'s heading. Looking for wherever the compass has been pointing all along.' },
      { art: '/story/act4/26_search_line.jpg', when: { flag: 'composition', is: 'couple' }, kind: 'narration', text: 'The storm\'s tail floods the creek at dusk, and in the scramble to move the camp uphill, a voice goes missing. Skip. The little robot who narrates EVERYTHING — silent. And the mud by the north path says WHERE, and the where stops both your hearts: small wooden footprints, heading inland. Toward the quiet camp. Toward the compass\'s heading.' },
      ...
      { kind: 'narration', when: { flag: 's4_search', is: 'oneline' }, text: 'One line, hands linked — the boat-deck line all over again, HOLD EACH OTHER made flesh. The line walks the compass heading through the dark, nobody let go of, until the sound of falling water grows a second sound inside it: a small voice, singing to itself to be brave.' },
      { art: '/story/act4/27_glow_behind_falls.jpg', when: { flag: 'composition', is: 'family' }, kind: 'narration', text: 'THE WATERFALL. And behind the water, where no map on this island ever put anything: a glow. Warm. Steady. Firelight — coming from inside the falls. The youngest\'s voice is coming from the light. And the youngest\'s voice says the impossible thing, clear over the water: "IT\'S OKAY! COME SEE! SOMEBODY LIVES HERE! SOMEBODY KIND!"' },
      { art: '/story/act4/27_glow_behind_falls.jpg', when: { flag: 'composition', is: 'couple' }, kind: 'narration', text: 'THE WATERFALL. And behind the water, where no map on this island ever put anything: a glow. Warm. Steady. Firelight — coming from inside the falls. Skip\'s voice is coming from the light — small, and for once not narrating, just calling. And then the impossible thing, clear over the water: "IT\'S OKAY! COME SEE! I FOUND THE BEFORE!"' },
    ],
    instrument: {
      toolId: 'barometer', toolName: 'The Barometer',
  ...
  M({
    ...
    opening: [
      ...
      { kind: 'narration', text: 'On the workbench wall, burned into a plank in a young hand, the words the family has been living without knowing it: A FAMILY THAT LOVES TOGETHER. A FAMILY THAT WORKS TOGETHER. A FAMILY THAT SURVIVES TOGETHER — the thing the boy who carved it never had, and built nine friends rather than stop believing in. And beneath the plank, two things kept side by side: a photograph — the same four sun-squinting faces from the tackle box, a boy and his people on a boat — and a ledger in a different, older hand, and the ledger tells the whole story to whoever reads page one.' },
      { kind: 'robot', who: 'hollow', text: '(from the cave mouth — the whole Jumble crowding behind him, and none of them surprised) …So. You found the Before. (he steps in, and for once the tall quiet one talks long) His name was Elias. A boy — younger than your youngest — put off a boat that never came back for him, twice, until he stopped waiting for one. He had no family to lose, so he built the ones he wanted, out of storm-wood, one at a time, and taught each of us the one thing he most wished someone had taught HIM. Thirty-one lessons. He wrote them down so they\'d outlast his hands — and they did. And the name on the wall? Weeks ago, the night of the first big storm, another family off your ship lost their quiet one — the middle Brennan boy, nine years old. Barnacle found him. The Collier kept the fire. The boy scratched his name so the cave would remember him — the way this cave has remembered every lost child since the first one built it. The fire you are standing beside was laid ready before any of you were born. Elias left it laid for whoever the sea sent next.' },
      { kind: 'robot', who: 'skip', text: '(very small) The signal fire was Elias\'s idea. We keep it lit-ready — he said someone should always be able to find the way in, even after. He made me LAST. Out of the leftover pieces. He said— (the wooden-crack voice) —he said the smallest scraps hold the most love per splinter, because you have to choose every single one.' },
      { kind: 'narration', text: 'And on the workbench, the boy\'s notebooks — a small tight hand going back to 1930, the earliest pages a child\'s. Ninety-seven tools written out, each with the name of the friend it was for. And on the last page, a list of thirty-one things, and under it, in the same hand: "For whoever the sea sends. Give them everything. They\'re why we practice." The lessons kept you. The family sits down together in the cave that built the ones who saved them, the storm riding itself out beyond the falls, and lets the truth be all of it at once: the loneliest story on this island, and the reason they\'re alive inside it.' },
    ],
    instrument: {
      toolId: 'mending_bench', toolName: 'The Mending Bench',
  ...
  M({
    ...
    opening: [
      { art: '/story/act5/29_plank_names.jpg', kind: 'narration', text: 'The morning after the cave, the Collier stands the family in front of the village and says the sentence the whole island has been building toward: "The Elders met. The trials begin. Five of them — the same five every Jumble passed. Finish, and you are one of us. And one of us…" (he looks at the sea) "…gets taken home."' },
      { kind: 'robot', who: 'collier', text: 'Trial one: THE RAFT. The plans are Elias\'s, from the cave, drawn for exactly this — a boy who never sailed off this island drawing the boat that would carry someone who could. Every plank gets a name before it gets a lashing: the name of something this family survived. You don\'t float on wood. You float on what the wood MEANS. That\'s not poetry. That\'s engineering. His kind.' },
  ...
  M({
    ...
    opening: [
      ...
      { kind: 'choice', id: 's5_letter', prompt: 'The feast quiets. The bottles wait. Who does your family write to?',
        options: [
          { id: 'elias', label: 'To Elias — the boy who built them, a lifetime too late to thank' },
          { id: 'selves', label: 'To yourselves, one year from now — from the people you became here' },
        ] },
      { kind: 'narration', when: { flag: 's5_letter', is: 'elias' }, text: 'The letter to Elias takes four hands and one page: what the lessons did, what the robots became, what the family knows now that the dock family didn\'t. Hollow seals it. The Jumble walks it to the water together, every lantern lit, and the tide takes it toward wherever a boy who was never fetched finally gets to go. Nobody says a word the whole way back. Nobody needs to.' },
  ...
  M({
    ...
    opening: [
      { art: '/story/act5/30_totem.jpg', kind: 'narration', text: 'Trial four convenes at the village totem — the tall carved post where every Jumble\'s name lives, cut deep by the Collier\'s own hands. Tonight the whole village stands in a ring, lanterns up, and the Elders speak the old words: "Who vouches?"' },
      { kind: 'robot', who: 'skip', text: 'I DO. (he is standing on a stump to be taller and still isn\'t, and has never cared less) I vouched on day one before I asked permission and I vouch LOUDER now. They put out my hand with THEIR hands. They named their planks. They found the Before and didn\'t look away. They are the FIRST family in a hundred years of ocean to do all thirty-one things, and I\'m the one who gets to say it because I\'m the sponsor: THEY\'RE OURS.' },
      { kind: 'narration', text: 'The Collier steps to the totem with his carving knife. And one by one — each family member\'s name, spoken by the youngest robot and cut by the oldest — the family goes into the wood, into the village, into the long record of the island. Below the names, the Collier cuts one more line, his splices steady: ELIAS\'S PEOPLE — ALL OF US.' },
  ...
  M({
    ...
    opening: [
      { art: '/story/act5/31_compass_hands.jpg', kind: 'narration', text: 'Dawn. The signal fire takes on the first spark — the family\'s spark, struck with the fire-steel from the sea chest — and climbs the cliff-top wood like it was born knowing the way. Smoke, straight and tall, into a clear 1955 sky. And before the morning is out, the sea answers: a shape on the horizon. A boat. Real. Coming.' },
      ...
      { kind: 'choice', id: 's5_boat_name', prompt: 'The raft built from every named plank of this crossing. Her name:',
        options: [
          { id: 'driftwood', label: 'THE DRIFTWOOD — the word the sea throws away, on the thing that carries you home' },
      ...
      { kind: 'narration', when: { flag: 's5_boat_name', is: 'driftwood' }, text: 'THE DRIFTWOOD, painted along the hull in somebody\'s very best letters. And Skip — who narrates everything — says nothing at all. He has just heard his friend\'s name, the one the sea threw away, said with love, by a family, on the side of the thing that carries them home. He sits down in the sand. The whole village pretends not to notice. The Collier has to oil something that isn\'t squeaking. Twice.' },
      ...
      { art: '/story/act5/32_fire_lit.jpg', kind: 'narration', text: 'The boat anchors off the cove, and the man who rows ashore walks up the beach like he has been looking for this island his whole life. A doctor\'s bag in one hand. A small boy asleep against his back, flushed with the kind of fever 1955 has no answer for. "Malakor," he says, and his voice breaks on the gratitude. "Dr. Lance Malakor. Thank God — thank God somebody\'s alive out here." He is kind. He is capable. He gets every last castaway aboard. And as the family waits at the water\'s edge, Skip presses something into their hands: the compass. Polished. The needle swings once, twice — and settles, pointing not at the island, not at the boat, but square at the middle of the family standing together on the sand. "He fixed it years ago," Skip says. "It never pointed north. It points HOME. That\'s why it kept pointing at the village — and look what it points at now." The family that boards the boat is not the family that fell off one. Behind them the signal fire burns tall, and the doctor stands a long time at the treeline, looking inland, the way a man looks at an answer. A family that loves together. A family that works together. A family that survives together. THE END — and the beginning of another story, on this island, which is the same thing.' },
    ],
    instrument: {
